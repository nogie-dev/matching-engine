...
| Raw match logs, sqlc, migrations | `context/docs/matching-log.md`, `context/considerations/matching-log-backpressure.md`, `internal/matchlog`, `sqlc.yaml`, `db/query`, `db/migrations` | Keep this repo to raw append-only logs; leave ETL and analytics DB out of scope | `sqlc generate` and `go test ./internal/matchlog/...` pass |
